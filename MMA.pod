...
is populated.  If the operand is omitted or zero, the main block
...
These operations return I<undef> if:
    ...
    * if the selected element was created by a store to a higher-
        numbered element or a I<storesize> with a large number of
...
These operations return false if:
    ...
    * for an MM_ARRAY, if the selected element has been deleted, or
    * if the selected element was created by a store to a higher-
        numbered element or a I<storesize> with a large number of entries,
...
to CPAN in 1H09 that will be an example of using the I<shiftCount> 
