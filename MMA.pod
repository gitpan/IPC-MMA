...
numbers, fixed-length strings/records, and booleans at one bit per
array element.
...
not.  The "mma_" version should only be used between explicit
...
stored.  Terms from outside Perl that can be applied to entries
in such an array include "floating point", "real", and "long long".
Depending on how your Perl system was built, INT_ARRAYs and DOUBLE_ARRAYs
may have the same maxima for integer values.

=item I<MM_BOOL_ARRAY>

values are stored as single bits packed in the array.

=item I<a positive integer>
...
representation is shorter than the length specified here, the stored
value is padded out with ASCII NUL bytes (binary zeroes) after the
last byte.  If the string representation of a Perl scalar being stored
is longer than the length specified here, the rightmost bytes are
...
length specified for the array.  This option is useful for "records"
...
NUL (zero) character within the stored data, or the specified length
if there are no NULs in the stored data.

=back

=item B<$hash = mm[a]_make_hash($mm [, alloc_entries]);>
...
I<$lockTiedScalar> should only be used between explicit calls to
...
I<@lockTiedArray> should only be used between explicit calls to
...
I<%lockTiedHash> should only be used between explicit calls to
...
is extended to accommodate it.  Any elements between the former array
...
I<mm_array_status> call) is decremented by the number of entries
added to the array.
...
to wait for any and all other I<RD> locks to be released before the
I<RW> lock is granted.
...
executing the splice, I<splice> returns an empty list in list context,
...
I<scalar(%tiedHash)> doesn't work under Perl 5.6.x: use
I<scalar(keys(%tiedHash))> instead.
...
block, as described for I<mm_make_array>.  Clearing an array also
clears its I<shiftCount>.
...
The optional I<$entries> determines the allocated size of the pointer
...
calls similarly between I<mm_lock($mm, MM_LOCK_R?)> and
...
I<IPC::MM::Hash>) is unreliable because another process can delete
...
A read-locked I<keys> operation is probably OK, but a locked
I<each> sequence (or a locked sequence using I<mma_hash_next_key>)
can lock out a process requesting a write lock for a long time.

In general the I<mm[a]_hash_get_entry> call is a better choice than
I<mm[a]_hash_next_key> to traverse a hash.
...
any hash entries, or if other processes may add or delete hash entries
during the sequence, the whole sequence should be explicitly locked.
Deletions are OK as long as the code decrements C<$i> (or avoids
incrementing C<$i>) to avoid skipping an entry.
...
describing result of the most recent shared memory operation.
I<mm> routines post to this facility, and so do some IPC::MMA routines.
...
array, the sum of the first two returned values is the same
...
to CPAN soon, that will be an example of using the I<shiftCount>
facility of IPC::MMA arrays.


=head2 Hash Key Ordering

IPC:MMA hashes (like IPC:MM BTrees) maintain their keys in the
order that is natural for I< unsigned byte arrays>.  IPC::MMA uses
the C library routine I<memcmp> for key comparisons.  No international,
...
unsigned integers.  For signed 32-bit integers (on a Perl system
built for 32-bit integers) try this when storing:
...
a simple internal organization that is optimized for key-value pairs
in an internal memory of limited extent.
...
time so that reallocation doesn't happen too often.
...
allocates, so IPC::MMA calls the I<mm> routine I<mm_sizeof>
...
of the hash.  A binary search is slower than a hashing algorithm for
small numbers of entries, but that's not the interesting end of
the distribution of applications.  If there are more than a hundred
entries a binary search is competitive with a true hash in
performance, and for thousands of entries a binary search is
faster.  Also the memory requirements of this structure are less
than for a BTree or a true hash.

IPC::MMA handles zero-length values (empty strings)  by leaving the
value pointer in the I<key block> zero (NULL).  IPC::MMA handles a
zero-length key (there can be only one such key in a hash) by making
the I<key block> contain just the number of bytes in the value
pointer.  A request to store either an undefined key or an undefined
value is rejected with an error message.
...
aliases for IPC::MM direct calls and tie-class-names, but no one
