...
rather than just ASCII strings.  IPC::MMA hashes are like IPC::MM 
...
numbers / large integers, fixed-length scalars/records, and booleans
...
But sometimes Perl scripts that use IPC::MMA "have a larger agenda" 
that require them to do locking and unlocking themselves.  The most 
obvious example is updating a record in a shared array or hash:
...
Unless the script using IPC::MMA can lock the sequence like this
to prevent other processes from accessing the shared hash record
during the sequence, such updating is unreliable.
...
These fetch the key and value of an entry in a shared hash, given its
index within the sorted array of keys.  (An IPC::MMA hash maintains
its entries in sorted order on its keys.)  These operations return
I<undef> if:
...
deleted and returned.  If I<$delCt> is zero, splice returns I<undef>.

If there are more than 3 operands, the values in I<list> are inserted
in the array starting at element I<$ix>.  Error messages will oocur
...
If I<$ix> is zero, the splice operation affects the array's shift 
count, which can be accessed via the I<mm_array_status> call.
...
I<$shiftCount> is a count of how many entries have been shifted out 
of the array.  I<$shiftCount> is negative if more elements have been 
unshifted in than shifted out.  I<$type> and I<$option> are as 
described for I<mm_make_array>.  I<$array> is a return value
from I<mm_make_array>.
...
=item B<$alloc_size = mm_alloc_size ($mm);>

This call is provided by IPC::MMA, and returns the number of bytes
that I<mm> uses as its "chunk size" for memory allocation.  The
test routines run during IPC::MMA installation use this value to
compute how much memory should be taken from and returned to the
I<mm_available> value by each operation being tested.  It returns
8 on the author's platform.  It may return a higher value on
other platforms.
...
The following aliases are included because the author became 
enamored of the I<ALIAS:> feature of the I<XS> interface and 
wanted everything to be complete and orthogonal:
